...
**Conclusión provisional:** si se quiere conservar Inno Setup y el ZIP portable tal como están, NetSparkleUpdater encaja mejor por ser agnóstico del instalador, a costa de tener que generar y publicar un appcast propio (no lee la API de Releases de GitHub de forma nativa). Si se está dispuesto a migrar el empaquetado completo a cambio de delta updates y lectura directa de GitHub Releases, Velopack es la opción más nueva y activamente mantenida, pero implica reemplazar el instalador actual y auditar dónde persisten hoy las preferencias del usuario. Ninguna de las dos es "la" estándar del ecosistema del modo en que Inno Setup lo es para instaladores.

**Pendiente:** decidir alcance (solo aviso vs. descarga asistida), elegir entre conservar Inno Setup (NetSparkleUpdater + appcast propio) o migrar el empaquetado (Velopack + `GithubSource`), y promover a un plan de versión cuando se aborde.
