...
**Estudio del ecosistema (7 de agosto de 2026):** no hay un único estándar de facto tan indiscutido como Inno Setup lo es para instaladores; el panorama para .NET/Windows se reparte entre dos familias con filosofías distintas.

- **[Velopack](https://github.com/velopack/velopack)** (sucesor activo de Squirrel/Clowd.Squirrel): framework de instalación *y* actualización integrado, con `GithubSource` nativo para leer Releases de GitHub, paquetes delta (solo descarga el cambio entre versiones) y binario de empaquetado en Rust. **Trae su propio instalador** (`Setup.exe` per-user sin elevación a `%LocalAppData%`, o `.msi` per-machine con `--msi`), lo que en la práctica **reemplaza a Inno Setup** en vez de convivir con él; adoptarlo implicaría rehacer el empaquetado que hoy hacen `scripts/build-installer.ps1` y el ZIP portable. Soporta instalación desatendida con `Setup.exe --silent` y `--installto {dir}`. Sobre preferencias: como el update reemplaza por completo la carpeta de la app, exige guardar configuración fuera de esa carpeta (`%LocalAppData%\{packId}\..` o `%AppData%\{packId}` para datos que deban sobrevivir a una desinstalación) — sería necesario revisar dónde persiste hoy `Configuration`/`config.txt` para confirmar que ya vive fuera del árbol de publicación, o migrarla si no es así.
- **[NetSparkleUpdater](https://github.com/NetSparkleUpdater/NetSparkle)** (puerto .NET del Sparkle de macOS): **no reemplaza archivos de la app ni trae instalador propio**; es agnóstico del empaquetador — descarga y ejecuta el instalador externo que ya se tenga (Inno Setup, WiX, NSIS), por lo que conviviría con el pipeline actual de `build-installer.ps1` sin rehacerlo. Comprueba actualizaciones contra un *appcast* (XML/JSON) con versión y notas de la release, no contra la API de Releases de GitHub directamente — el generador (`netsparkle-generate-appcast`) no tiene salida nativa a GitHub Releases todavía (se discute como mejora pendiente en su propio repositorio), así que habría que publicar el appcast aparte (por ejemplo, generado por un workflow de CI y publicado como asset de la Release o vía GitHub Pages). Ofrece UI prearmada para WinForms/WPF (no WinUI 3 directamente) y también un modo `UserInteractionMode.DownloadAndInstall` para descarga silenciosa; la instalación silenciosa final depende de los flags del instalador que ya se use, es decir de los flags de Inno Setup (`/VERYSILENT`, etc.), no de NetSparkle.

Además de esas dos, existen otras dos vías nativas de Microsoft, descartadas para este proyecto pero registradas por completitud:

- **ClickOnce**: tecnología de despliegue de .NET con actualización automática incorporada, pero considerada legado en 2026 —poca personalización, encaja peor con prácticas de empaquetado modernas— y pensada para apps de un solo usuario firmadas contra un servidor propio, no contra Releases de GitHub.
- **MSIX + App Installer** (`.appinstaller` con `UpdateUri`): es el reemplazo oficial de Microsoft para ClickOnce y soporta actualización automática en segundo plano sin pasar por la Store, incluso para apps distribuidas fuera de ella («sideload»). Requeriría empaquetar como MSIX en lugar de (o además de) Inno Setup, y remonta a otro cambio de empaquetado tan grande como el de Velopack; no se investigó en profundidad porque el objetivo es conservar el instalador actual.

**Conclusión provisional:** no existe un único gold standard del modo en que Inno Setup lo es para instaladores; la elección depende de si se prioriza conservar el pipeline de empaquetado actual (NetSparkleUpdater, o la opción ligera de abajo) o adoptar el ecosistema completo de otra herramienta (Velopack, MSIX). Para este proyecto, conservar Inno Setup y el ZIP portable pesa más que ganar delta updates o integración nativa con Store/App Installer.
