...
Este documento define el contrato persistente del esquema 3 y las operaciones que pueden modificarlo. El archivo es portable entre instalaciones de RightKeyboard, pero sus distribuciones solo se activan si también están instaladas en el Windows de destino.
...
- Los esquemas anteriores al 2 y posteriores al 3 se rechazan. En particular, una versión futura nunca se intenta leer como esquema 3.
- Las colecciones `null` del esquema 3 se tratan como vacías. JSON inválido, tipos incompatibles, identidades vacías, duplicados sin distinguir mayúsculas, referencias inexistentes y estados contradictorios se rechazan antes de modificar preferencias.
...
La acción guarda un esquema 3 válido con `devices`, `mappings` e `ignoredDeviceIds` vacíos. Por lo tanto elimina:
