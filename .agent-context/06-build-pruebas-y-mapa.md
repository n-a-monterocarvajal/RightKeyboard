...
Tras 1.6.1: 246/246 pruebas NUnit y 2/2 pruebas WinUI de árbol visual. El script ejecuta NUnit con `dotnet test` y la aplicación de prueba WinUI directamente, porque el repositorio mantiene VSTest para NUnit y Microsoft Testing Platform para el proceso XAML.
...
|---|---|---|---|
| CI | `.github/workflows/ci.yml` | push a `master`, cada pull request, manual (`workflow_dispatch`) | `dotnet restore` + `dotnet build -c Release` (solución completa; WinUI mapea a x64) + `scripts/run-tests.ps1`: 246 NUnit y 2 pruebas WinUI. Advertencias como errores. Sin artefactos. |
...
| `RightKeyboard.NUnit/` | 246 pruebas unitarias/interop/DTO, arquitectura compartida y dibujo WinForms tras 1.6.1 |
